...
function getNotificationTitleAndBody(
  ...
  const creditsEarned = data.creditsEarned != null ? String(data.creditsEarned) : '';
  ...
  switch (type) {
    ...
    case 'orderAccepted':
      return {
        ...
        body: publicCode
          ? `Your order #${publicCode} has been accepted${deliveryDate ? ` – delivery on ${deliveryDate}` : ''}.`
          : `Your order has been accepted${deliveryDate ? ` – delivery on ${deliveryDate}` : ''}.`,
      };
    case 'orderRejected':
      return {
        title: '❌ Order Rejected',
        body: publicCode
          ? `Your order #${publicCode} was rejected${rejectionReason ? `: ${rejectionReason}` : ''}.`
          : `Your order was rejected${rejectionReason ? `: ${rejectionReason}` : ''}.`,
      };
    case 'orderDelivered':
      return {
    ...
    case 'bookingAccepted':
      return {
        ...
        body: publicCode
          ? `Your booking #${publicCode} has been accepted${deliveryDate ? ` – ${mealTime ? mealTime + ' on ' : ''}${deliveryDate}` : ''}.`
    ...
    case 'bookingRejected':
      return {
        ...
        body: publicCode
          ? `Your booking #${publicCode} was rejected${rejectionReason ? `: ${rejectionReason}` : ''}.`
          : `Your booking was rejected${rejectionReason ? `: ${rejectionReason}` : ''}.`,
      };
    case 'bookingCancelled':
      return {
        title: '🚫 Booking Cancelled',
        body: publicCode
          ? `Your booking #${publicCode} was cancelled${cancellationReason ? `: ${cancellationReason}` : ''}.`
    ...
    case 'new_review':
      return {
        title: '⭐ New Review',
        body: `You received a new review${publicCode ? ` for order #${publicCode}` : ''}.`,
      };
    case 'review_update':
      return {
        title: '✏️ Review Updated',
        body: `A review was updated${publicCode ? ` for order #${publicCode}` : ''}.`,
      };
    case 'review_deleted':
      return {
        title: '🗑 Review Removed',
        body: `A review was removed${publicCode ? ` for order #${publicCode}` : ''}.`,
      };
    case 'vendor_response':
      return {
...
export const sendPushOnNotificationCreate = functions.firestore
  ...
  .onCreate(async (snap: functions.firestore.DocumentSnapshot) => {
    ...
    const {title, body} = getNotificationTitleAndBody(notificationType, notification);
